...
我在核磁共振机器上躺着的 10 分钟里想到这个项目，之后用约 10 天把它从想法做成了可运行的 Windows 桌宠。最初我只想做一个喝水和休息提醒工具，但普通通知很容易在忙碌时被顺手忽略，所以我选择加入一个低打扰、可拖动的桌宠，让提醒变成有人陪伴的日常反馈。

项目名称 `Ikuyo Pet` 也有一层日语双关：喜多郁代的名字读作 `Kita Ikuyo`；日语里的「行くよ（iku yo）」则有“要出发啦、走啦、开始行动吧”的语气。喝水、休息和重新开始工作，正好都可以用这种轻快而主动的方式提醒。

我选择喜多郁代，是因为她的元气、亲和力和舞台节奏感适合健康提醒：不把用户训诫成被动执行者，而是用气泡、动作和鼓励语陪着用户找回自己的工作节奏。项目中的气泡、皮肤处理、交互逻辑和文案由我持续迭代完成；本项目是非官方同人创作，与原作版权方无关联，也不主张拥有原作角色、名称和世界观的权利。
...
当桌宠被关闭、隐藏或暂时不可用时，提醒会回退到 Windows 原生通知。下面的截图分别记录喝水提醒、休息提醒和通知回退的实际效果：

<table>
<tr>
<td><img src="./docs/assets/kita/win通知喝水.png" alt="Windows 喝水通知" width="330"></td>
<td><img src="./docs/assets/kita/win通知休息.png" alt="Windows 休息通知" width="330"></td>
<td><img src="./docs/assets/kita/windows通知休息.png" alt="Windows 通知回退示例" width="330"></td>
</tr>
</table>
...
| [win通知喝水.png](./docs/assets/kita/win通知喝水.png) | Windows 原生喝水通知：验收桌宠不可用时仍能收到提醒，并保留 Ikuyo Pet 标识。 |
| [win通知休息.png](./docs/assets/kita/win通知休息.png) | Windows 原生休息通知：验收离屏活动提醒在隐藏桌宠时的回退渠道。 |
| [windows通知休息.png](./docs/assets/kita/windows通知休息.png) | Windows 通知回退样例：验收完成反馈与系统通知的实际排版。 |
