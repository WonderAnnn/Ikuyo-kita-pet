...
| P-02 | 首次点击后在 5 秒内连续点击 10 次 | 只保留一个气泡和一个计时器；窗口内只更新待显示的最后一条文案，不叠加气泡 | 待测 |
| P-03 | 点击后持续观察气泡 | 气泡淡入和淡出各不超过 0.5 秒；普通气泡约 4 秒后开始淡出 | 待测 |
| P-04 | 5 秒窗口结束后继续观察 | 若窗口内有点击，显示最后一次点击的文案；窗口结束后重新点击可立即开始下一轮 | 待测 |
| P-05 | 按住角色移动超过系统拖动阈值后释放 | 桌宠移动且不弹普通点击文案 | 待测 |
| P-06 | 拖到屏幕四边 | 松开后保持在当前工作区内 | 待测 |
| P-07 | 观察立绘 | 透明背景正常，`Stretch=Uniform`，人物比例未拉伸 | 待测 |
