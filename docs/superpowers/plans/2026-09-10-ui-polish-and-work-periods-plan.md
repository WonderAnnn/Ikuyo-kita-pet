...
> **面向 AI 代理的工作者：** 必需子技能：使用 superpowers:subagent-driven-development（推荐）或 superpowers:executing-plans 逐任务实现此计划。步骤使用复选框（`- [x]`）语法跟踪进度。
...
- [x] 编写失败测试：周范围为选定日期前 6 天至次日，月范围为前 29 天至次日；ViewModel 暴露北京时间范围文案。
- [x] 运行 Core 与 Infrastructure 定向测试确认红灯。
- [x] 实现滚动日期范围和 `WorkStatisticsRangeText`。
- [x] 在总有效工作后显示范围文本。
- [x] 运行定向测试确认通过。
...
- [x] 编写失败契约：ComboBox 使用透明自定义 ToggleButton 模板，导航样式关闭 FocusVisualStyle。
- [x] 运行契约测试确认红灯。
- [x] 实现 A 方案样式，保留浅粉色选中项和细粉色焦点边框。
- [x] 运行契约测试确认通过。
...
- [x] 编写失败测试：全量会话可读，主页面同时暴露累计和今日文案。
- [x] 运行测试确认红灯。
- [x] 增加仓储全量读取（旧实现默认抛出 NotSupported），ViewModel 叠加实时秒数。
- [x] 更新今日卡片布局。
- [x] 运行定向和全量测试确认通过。
...
- [x] 编写失败契约：设置页绑定版本文本且项目版本为 `0.1.1`。
- [x] 运行测试确认红灯。
- [x] 添加单一版本源和设置页底部文本。
- [x] 运行测试确认通过。
